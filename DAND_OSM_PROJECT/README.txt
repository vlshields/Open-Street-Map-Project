...
3) I ran into an issue where some of the values in element.attrib where missing in the nodes. Specifically, there were a few nodes missing the values 'user' and 'uid'. You can see how the issue was handled by following the comments in data.py. I know it probably wasn't the best way to handle the problem, but there where only four nodes that had this issue.
